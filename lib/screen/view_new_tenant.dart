...
import 'package:el_pg_app/screen/view_image.dart';
...
class ViewTenant extends StatefulWidget {
  ...
  ViewTenant(this.pgId, this.tenantId);
...
class _ViewTenantState extends State<ViewTenant> {
  ...
  UpdateTenantRequestModel _requestModel = UpdateTenantRequestModel();
  ...
  getTenantDetails() async {
    ViewTenantRequestModel requestModel = ViewTenantRequestModel();
    requestModel.pgId = widget.pgId;
    requestModel.tenantId = widget.tenantId;
    ViewTenantResponse viewTenantResponse =
        await Repository().getTenantDetails(requestModel);
    if (viewTenantResponse != null) {
      if (viewTenantResponse.status) {
        tenantData = viewTenantResponse.data;
        getProofTypes();
      } else {}
    } else {}

    _fullNameController.text = tenantData.name;
    _numberController.text = tenantData.mobile;
    _joinDateController.text =
        DateFormat('dd-MMM-yyyy').format(tenantData.joinDate);
    _exitDateController.text = tenantData.exitDate != null
        ? DateFormat('dd-MMM-yyyy').format(
            DateFormat('yyyy-MM-dd').parse(tenantData.exitDate.toString()))
        : '';
    _emailController.text = tenantData.email;
    _add1Controller.text = tenantData.addressLine1;
    _add2Controller.text = tenantData.addressLine2;
    _cityController.text = tenantData.city;
    _stateController.text = tenantData.state;
    _pinCodeController.text = tenantData.pincode;
    _roomNoController.text = tenantData.roomName;
    _rentController.text = tenantData.rent;
    _maintenanceController.text = tenantData.maintenance;
    _depositController.text = tenantData.deposit;
    _contactNameController.text = tenantData.emergencyContactName;
    _contactNumberController.text = tenantData.emergencyContactNumber;
    _emergencyEmailController.text = tenantData.emergencyContactEmail;
    _employerNameController.text = tenantData.employerName;

    for (int i = 0; i < stateList.length; i++) {
      if (stateList[i].code == tenantData.state) {
  ...
  onClickOnBoard() async {
    _requestModel.tenantId = widget.tenantId;
    if (_formKey.currentState.validate()) {
      _formKey.currentState.save();
      if (_proofTypeDoc1 == null) {
        ...
        AddTenantResponse response =
            await Repository().updateTenant(_requestModel);
        if (response != null) {
          if (response.status) {
            isEditable = false;
  ...
  void getProofTypes() async {
    ...
    if (response != null) {
      if (response.status) {
        ...
        for (int i = 0; i < _proofList.length; i++) {
          if (_proofList[i].id.toString() == tenantData.documentType1) {
            _proofTypeDoc1 = _proofList[i];
            _requestModel.idType1 = _proofTypeDoc1.id.toString();
          }
          if (_proofList[i].id.toString() == tenantData.documentType2) {
            _proofTypeDoc2 = _proofList[i];
            _requestModel.idType2 = _proofTypeDoc2.id.toString();
          }
        }
      }
    }
  ...
  showConfirmationDialog() async {
    bool action = await showCupertinoDialog(
        context: context,
        builder: (context) {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
            backgroundColor: Colors.blue[600],
            title: Text('Update Guest Details', textScaleFactor: 1.0),
            leading: InkWell(
              onTap: () {
                if (isEditable) {
                  showConfirmationDialog();
        ...
        floatingActionButton: !isEditable
            ? FloatingActionButton.extended(
                onPressed: () {
                  Utility.showSuccessSnackBar(
                      _scaffoldKey, 'You can edit now.');
                  setState(() {
                    isEditable = true;
                  });
                },
                label: Text('Edit', textScaleFactor: 1.0),
                      ...
                      child: InkWell(
                        ...
                        child: Container(
                          ...
                          child: _requestModel.profileImg != null
                              ...
                              : tenantData.profileImage.isNotEmpty
                                  ? InkWell(
                                      onTap: () {
                                      ...
                                      child: CachedNetworkImage(
                                        imageUrl: tenantData.profileImage,
                                        fit: BoxFit.cover,
                                      ))
                                  : Icon(
                                      Icons.camera_alt,
      ...
      content: Column(
        ...
        children: <Widget>[
          ...
          _requestModel.idDoc1 != null
              ...
              : tenantData.idDocument1 != null &&
                      tenantData.idDocument1.isNotEmpty
                  ? Container(
                      child: Stack(
                        children: <Widget>[
                          InkWell(
                            onTap: () {
                              Navigator.of(context).push(MaterialPageRoute(
                                  builder: (context) =>
                                      ViewImage(url: tenantData.idDocument1)));
                            },
                            child: Container(
                              margin: const EdgeInsets.all(10),
                              padding: const EdgeInsets.all(5),
                              ...
                              child: Column(
                                ...
                                children: <Widget>[
                                  ...
                                  Text(
                                    path.basename(tenantData.idDocument1),
                                    textAlign: TextAlign.center,
                                    textScaleFactor: 1.0,
                                  )
                                ],
                          ...
                          InkWell(
                            onTap: () {
                              if (isEditable) {
                                setState(() {
                                  _requestModel.document1_del =
                                      tenantData.idDocument1.replaceAll(ApiClient.BASE_URL_IMAGE,"") ?? '';
                                  tenantData.idDocument1 = '';
                                  _requestModel.idDoc1 = null;
                                });
                              }
                            },
          ...
          _requestModel.idDoc2 != null
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            Text(path.basename(_requestModel.idDoc2.path),
                                textAlign: TextAlign.center,
                                textScaleFactor: 1.0)
                          ],
                        ),
              ...
              : tenantData.idDocument2 != null &&
                      tenantData.idDocument2.isNotEmpty
                  ? Container(
                      child: Stack(
                        children: <Widget>[
                          InkWell(
                            onTap: () {
                              Navigator.of(context).push(MaterialPageRoute(
                                  builder: (context) =>
                                      ViewImage(url: tenantData.idDocument2)));
                            },
                            child: Container(
                              margin: const EdgeInsets.all(10),
                              padding: const EdgeInsets.all(5),
                              ...
                              child: Column(
                                ...
                                children: <Widget>[
                                  ...
                                  Text(path.basename(tenantData.idDocument2),
                                      textAlign: TextAlign.center,
                                      textScaleFactor: 1.0)
                                ],
                              ),
                          ...
                          InkWell(
                            onTap: () {
                              if (isEditable) {
                                setState(() {
                                  _requestModel.document2_del =
                                      tenantData.idDocument2.replaceAll(ApiClient.BASE_URL_IMAGE,"") ?? '';
                                  tenantData.idDocument2 = '';
                                  _requestModel.idDoc2 = null;
                                });
                              }
                            },
      ...
      content: Column(
        ...
        children: <Widget>[
          ...
          _requestModel.employeeId != null
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            Text(path.basename(_requestModel.employeeId.path),
                                textAlign: TextAlign.center,
                                textScaleFactor: 1.0)
                          ],
                        ),
              ...
              : tenantData.employeeIdProof != null &&
                      tenantData.employeeIdProof.isNotEmpty
                  ? Container(
                      child: Stack(
                        children: <Widget>[
                          Container(
                            ...
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                Text(path.basename(tenantData.employeeIdProof),
                                    textAlign: TextAlign.center,
                                    textScaleFactor: 1.0)
                              ],
                            ),
                  ...
                  Expanded(
                    child: CupertinoPicker(
                      ...
                      children:
                        ...
                        return Center(
                          child:
                              Text(stateList[index].name, textScaleFactor: 1.0),
                        );
                      }),
                      onSelectedItemChanged: (index) {
                        selectedIndex = index;
